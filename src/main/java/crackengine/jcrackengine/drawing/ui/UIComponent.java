package crackengine.jcrackengine.drawing.ui;

import crackengine.jcrackengine.drawing.GameObject;
import crackengine.jcrackengine.drawing.interfaces.Drawable;
import crackengine.jcrackengine.drawing.interfaces.Updatable;

public abstract class UIComponent extends GameObject implements Drawable, Updatable {
    /*add here things specific for UI components, I don't know should I put here yet*/
}
